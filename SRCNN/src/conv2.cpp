           ...
           conv2o_t output_ftmap[N2][H][W])
{
	...
	#pragma HLS ARRAY_PARTITION variable=output_fm_buffer dim=3 type=cyclic factor=15

	static conv1o_t input_fm_buffer[C2_ID][C2_TH + (2 * P2)][W + (2 * P2)];
	#pragma HLS bind_storage variable=input_fm_buffer type=RAM_2P impl=BRAM

	#pragma HLS ARRAY_PARTITION variable=input_fm_buffer dim=2 type=cyclic factor=2
	#pragma HLS ARRAY_PARTITION variable=input_fm_buffer dim=3 type=cyclic factor=15
